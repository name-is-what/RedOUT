...
class HRNEncoder(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        for _ in range(self.num_convs):
            ...
            # print(f"x: {x.shape}, edge_index: {edge_index.shape}, size: {size}")
            ## x: torch.Size([2010, 53]), edge_index: torch.Size([2, 2010]), size: tensor([2010, 1596], device='cuda:0')
            ## x: torch.Size([1817, 53]), edge_index: torch.Size([2, 1817]), size: tensor([1817, 1570], device='cuda:0')
            ## dataset==> x: torch.Size([1823, 53]), edge_index: torch.Size([2, 1823]), size: tensor([1823, 1527], device='cuda:0')
