...
class HCL(nn.Module):
    ...
    @staticmethod
    def calc_loss_tree(x, x_aug, sym=True, temperature=0.2):   # sym=False
        batch_size, _ = x.size()
        x_abs = x.norm(dim=1)
        x_aug_abs = x_aug.norm(dim=1)
        # print(f"loss_tree: x.shape={x.shape}, x_aug.shape={x_aug.shape}")
        # loss_g: x.shape=torch.Size([53, 160]), x_aug.shape=torch.Size([53, 160])
        # loss_n: x.shape=torch.Size([729, 160]), x_aug.shape=torch.Size([729, 160])
        # loss_tree: x.shape=torch.Size([53, 160]), x_aug.shape=torch.Size([53, 160])
    ...
    def calc_loss_n(x, x_aug, batch, temperature=0.2):
        batch_size, _ = x.size()
        # print(f"loss_n: x.shape={x.shape}, x_aug.shape={x_aug.shape}")
        x_abs = x.norm(dim=1)
        x_aug_abs = x_aug.norm(dim=1)
    ...
    def calc_loss_g(x, x_aug, temperature=0.2):
        batch_size, _ = x.size()
        # print(f"loss_g: x.shape={x.shape}, x_aug.shape={x_aug.shape}")
        x_abs = x.norm(dim=1)
        x_aug_abs = x_aug.norm(dim=1)
